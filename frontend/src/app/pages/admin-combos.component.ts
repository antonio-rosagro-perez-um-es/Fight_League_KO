...
@Component({
  ...
  styles: [`
    ...
    .modal-overlay { align-items: center; background: rgba(0,0,0,.55); display: flex; inset: 0; justify-content: center; position: fixed; z-index: 100; }
    .modal-content { background: #06120a; border: 1px solid rgba(255,255,255,.12); border-radius: 18px; max-height: 86vh; max-width: 980px; overflow-y: auto; padding: 1.5rem; width: min(96vw, 980px); }
    ...
    .deleted { opacity: .55; }
    .row-actions { display: flex; flex-wrap: nowrap; gap: .5rem; white-space: nowrap; }
...
export class AdminCombosComponent {
  ...
  readonly combos$ = this.refresh$.pipe(
    switchMap(() => this.api.getAllCombosForAdmin().pipe(
      map((page) => [...page.content].sort((a, b) => Date.parse(b.createdAt) - Date.parse(a.createdAt)))
    ))
  );
